...
type UserCardProps = {
  ...
  const imageKeywordPairs = [
    ...
    { name: 'food', keywords: ['food', 'reastaurant','reastro','panipuri','dinner','lunch'] },
    { name: 'cart', keywords: ['order', 'amazon','flipkart'] },
    { name: 'grocery', keywords: ['grocery', 'dmart','mart'] },
    { name: 'list', keywords: ['list'] },
    { name: 'fruits', keywords: ['fruit', 'fruits','mangoes','mango','apple','banana'] },
  ...
  const findMatchingImage = (desc: string) => {
    ...
    const matchingImage = imageKeywordPairs.find(pair => pair.keywords.some(keyword => 
  ...
  return (
    ...
    >
      <img src={imagePath} alt="Logo" style={{ width: '80%', height: '80%', borderRadius: '50%' }} />
    </div>
  );
};
